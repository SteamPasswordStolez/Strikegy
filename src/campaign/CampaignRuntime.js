...
export class CampaignRuntime {
  ...
  // Patch 4.5 Pre Beta HF: instant dialogue interrupt + sticky waypoint preview.
  // - Objective step should set/preview the next navigation waypoint BEFORE any dialogue plays.
  // - If an objective completes while a line is still speaking, cut it immediately and advance.
  _interruptDialogueNow(reason=''){
    try{
      this._dialogueGen = (Number(this._dialogueGen) || 0) + 1;
    }catch{ this._dialogueGen = 1; }

    // Immediately stop any currently playing voice + hide subtitles.
    try{ this.ui?.hideSubtitle?.(); }catch{}
    try{ (this.ttsManager || window.ttsManager)?.stop?.(); }catch{}
    try{ if (window.speechSynthesis?.speaking || window.speechSynthesis?.pending) window.speechSynthesis.cancel(); }catch{}

    // Detach future queued lines from the old chain.
    try{ this._dialogueChain = Promise.resolve(); }catch{}
    try{ this._dialoguePending = 0; }catch{}
  }

  _findNextNavStep(fromIndex){
    const steps = this.mission?.script || [];
    for(let i = Math.max(0, Number(fromIndex)||0) + 1; i < steps.length; i++){
      const s = steps[i];
      const t = String(s?.type || '');
      if((t === 'reach' || t === 'interact') && s?.trigger){
        return { idx:i, step:s };
      }
    }
    return null;
  ...
  _setStickyWaypointFromStep(navStep){
    try{
      const trgName = String(navStep?.trigger || '');
      if(!trgName) return;
      const trg = this.triggers?.[trgName];
      if(!trg || !Array.isArray(trg.pos)) return;
      this._stickyNav = {
        trigger: trgName,
        label: String(navStep?.label || trgName),
        type: String(navStep?.type || 'reach'),
        hintFar: String(navStep?.hintFar || '접근 중...'),
        hint: String(navStep?.hint || (String(navStep?.type)==='interact' ? 'E로 상호작용' : '이동')),
      };
      this._applyStickyWaypointNow();
    }catch{}
  }

  _applyStickyWaypointNow(){
    try{
      const sn = this._stickyNav;
      if(!sn) return;
      const trg = this.triggers?.[sn.trigger];
      if(!trg?.pos) return;
  ...
  _showCommsLine(line){
    ...
    this._queueDialogue(async () => {
      const myGen = Number(this._dialogueGen) || 0;
      ...
      // If we were interrupted before this line starts, skip it.
      if (myGen !== (Number(this._dialogueGen) || 0)) return;
  ...
  _enterStep() {
    ...
    if (step.type === 'objective') {
      ...
      // Patch 4.5 Pre Beta HF: preview the next navigation waypoint immediately.
      // (So UI updates first, then the chatter lines can play while the waypoint is visible.)
      try{
        const next = this._findNextNavStep(this.stepIndex);
        if(next?.step) this._setStickyWaypointFromStep(next.step);
        else this._stickyNav = null;
      }catch{ /* ignore */ }
  ...
  update(dt) {
    ...
    // Patch 4.5 Pre Beta HF:
    // Keep a "sticky" waypoint active across dialogue steps so the player sees
    // the waypoint panel/marker BEFORE and DURING the objective chatter.
    if (step.type !== 'reach' && step.type !== 'interact') {
      if (this._stickyNav && step.type !== 'cutscene' && step.type !== 'complete') {
        this._applyStickyWaypointNow();
      } else {
    ...
    if (step.type === 'reach') {
      ...
      if (d <= r) {
        // If the player completes the objective while a line is still speaking,
        // cut it instantly and move on.
        try{ this._stickyNav = null; }catch{}
        this._interruptDialogueNow('reach_complete');
    ...
    if (step.type === 'kill') {
      ...
      if (this.killCount >= need) {
        try{ this._stickyNav = null; }catch{}
        this._interruptDialogueNow('kill_complete');
    ...
    if (step.type === 'interact') {
      ...
      if (d <= r && this._consumeInteract()) {
        try{ this._stickyNav = null; }catch{}
        this._interruptDialogueNow('interact_complete');
    ...
    if (step.type === 'wait') {
      ...
      if (left <= 0) {
        try{ this._stickyNav = null; }catch{}
        this._interruptDialogueNow('wait_complete');
        if (step.objectiveKey) this._markObjectiveDone(step.objectiveKey);
    ...
    if (step.type === 'defend') {
      ...
      if (okTime && okKills) {
        try{ this._stickyNav = null; }catch{}
        this._interruptDialogueNow('defend_complete');
        if (step.objectiveKey) this._markObjectiveDone(step.objectiveKey);
