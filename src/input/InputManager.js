...
const preset = this.activePreset || this.settings.controlPreset || 'pc';
  ...
  // ---- virtual input API (used by MobileHUD) ----
  setVirtualMove(x, z){
    this.vMoveX = Math.max(-1, Math.min(1, x || 0));
    this.vMoveZ = Math.max(-1, Math.min(1, z || 0));
  }
  ...
  toggleSprint(){
    this.vSprintToggled = !this.vSprintToggled;
  }
...
const preset = this.activePreset || this.settings.controlPreset || 'pc';
    ...
    if(preset === "mobile"){
      moveX = this.vMoveX;
      moveZ = this.vMoveZ;
      sprintHeld = this.vSprintToggled;
    }
